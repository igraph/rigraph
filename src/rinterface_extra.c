...
void igraph_es_destroy_pv(void* pv_ptr)
{
  igraph_es_destroy((igraph_es_t*) pv_ptr);
}
...
igraph_bool_t R_igraph_interrupt_handler(void) {
...
SEXP Rx_igraph_add_vertices(SEXP graph, SEXP pnv) {
...
SEXP Rx_igraph_ecount(SEXP graph) {
...
SEXP Rx_igraph_delete_edges(SEXP graph, SEXP edges) {
...
SEXP Rx_igraph_delete_vertices(SEXP graph, SEXP vertices) {
...
SEXP Rx_igraph_is_directed(SEXP graph) {
...
SEXP R_igraph_get_diameter(SEXP graph, SEXP pdirected, SEXP punconnected,
  ...
  IGRAPH_R_CHECK(igraph_diameter(&g, Rf_isNull(pweights) ? 0 : &weights, &dialen, 0, 0, &res, 0, directed, unconnected));
...
SEXP R_igraph_farthest_points(SEXP graph, SEXP pdirected, SEXP punconnected,
  ...
  IGRAPH_R_CHECK(igraph_diameter(&g, Rf_isNull(pweights) ? 0 : &weights, &len, &from, &to, 0, 0, directed, unconnected));
...
SEXP Rx_igraph_running_mean(SEXP pdata, SEXP pbinwidth) {
...
SEXP Rx_igraph_cocitation(SEXP graph, SEXP pvids) {
...
SEXP Rx_igraph_bibcoupling(SEXP graph, SEXP pvids) {
...
SEXP R_igraph_shortest_paths(SEXP graph, SEXP pvids, SEXP pto,
  ...
  case 0:                       /* automatic */
    ...
    break;
  case 1:                       /* unweighted */
    IGRAPH_R_CHECK(igraph_distances(&g, NULL, &res, vs, to, mode));
    break;
  case 2:                       /* dijkstra */
    IGRAPH_R_CHECK(igraph_distances_dijkstra(&g, &res, vs, to, pw, mode));
    break;
...
SEXP R_igraph_get_shortest_paths(SEXP graph, SEXP pfrom, SEXP pto,
  ...
  case 0:                       /* automatic */
    ...
    break;
  case 1:                       /* unweighted */
    IGRAPH_R_CHECK(igraph_get_shortest_paths(&g, pw, verts ? &list : 0, edges ? &elist : 0, from, to, mode, pred ? &predvec : 0, inbound ? &inboundvec : 0));
    break;
  case 2:                       /* dijkstra */
    IGRAPH_R_CHECK(igraph_get_shortest_paths_dijkstra(&g, verts ? &list : 0, edges ? &elist : 0, from, to, pw, mode, pred ? &predvec : 0, inbound ? &inboundvec : 0));
    break;
...
SEXP Rx_igraph_read_graph_gml(SEXP pvfile) {
...
SEXP Rx_igraph_read_graph_dl(SEXP pvfile, SEXP pdirected) {
...
SEXP Rx_igraph_read_graph_graphdb(SEXP pvfile, SEXP pdirected) {
  igraph_t g;
  igraph_bool_t directed=LOGICAL(pdirected)[0];
  FILE *file;
  SEXP result;
...
SEXP Rx_igraph_write_graph_gml(SEXP graph, SEXP file, SEXP pid, SEXP pcreator) {
  igraph_t g;
  FILE *stream;
#if HAVE_OPEN_MEMSTREAM == 1
  char *bp;
...
SEXP Rx_igraph_write_graph_leda(SEXP graph, SEXP file, SEXP va, SEXP ea) {
  igraph_t g;
  FILE *stream;
#if HAVE_OPEN_MEMSTREAM == 1
  char *bp;
...
SEXP Rx_igraph_edges(SEXP graph, SEXP eids) {
  ...
  igraph_vector_int_init(&res, 0);
  IGRAPH_R_CHECK(igraph_edges(&g, es, &res, false));
...
SEXP Rx_igraph_rewire(SEXP graph, SEXP pn, SEXP pmode) {

  igraph_t g;
  igraph_integer_t n=(igraph_integer_t) REAL(pn)[0];
  igraph_edge_type_sw_t mode=REAL(pmode)[0];
  SEXP result;
...
SEXP Rx_igraph_read_graph_graphml(SEXP pvfile, SEXP pindex) {
  igraph_t g;
...
SEXP Rx_igraph_write_graph_graphml(SEXP graph, SEXP file, SEXP pprefixattr) {
...
SEXP Rx_igraph_mincut(SEXP graph, SEXP pcapacity) {
...
SEXP Rx_igraph_mincut_value(SEXP graph, SEXP pcapacity) {
...
SEXP Rx_igraph_edge_connectivity(SEXP graph, SEXP pchecks) {
...
SEXP R_igraph_community_edge_betweenness(SEXP graph, SEXP pweights,
  ...
  // FIXME: expose
  igraph_vector_t *pplengths = NULL;
  IGRAPH_R_CHECK(igraph_community_edge_betweenness(&g, &res, ppeb, ppmerges, ppbridges, ppmodularity, ppmembership, directed, ppweights, pplengths));
...
SEXP Rx_igraph_get_adjlist(SEXP graph, SEXP pmode, SEXP ploops, SEXP pmultiple) {
  ...
  igraph_vector_int_t neis;
  igraph_loops_t loops=(igraph_loops_t) REAL(ploops)[0];
  igraph_bool_t multiple=(igraph_bool_t) REAL(pmultiple)[0];
  ...
  for (igraph_integer_t i=0; i<no_of_nodes; i++) {
    igraph_neighbors(&g, &neis, i, mode, loops, multiple);
...
SEXP Rx_igraph_get_adjedgelist(SEXP graph, SEXP pmode, SEXP ploops) {
  ...
  igraph_integer_t no_of_nodes;
  igraph_vector_int_t neis;
  igraph_bool_t loops=(igraph_bool_t) REAL(ploops)[0];
  ...
  PROTECT(result=NEW_LIST(no_of_nodes));
  for (igraph_integer_t i=0; i<no_of_nodes; i++) {
    igraph_incident(&g, &neis, i, mode, loops);
    SET_VECTOR_ELT(result, i, R_igraph_vector_int_to_SEXP(&neis));
  }
  igraph_vector_int_destroy(&neis);
...
SEXP R_igraph_community_leading_eigenvector(SEXP graph, SEXP steps,
  ...
  if (0 != igraph_vector_int_init(&c_history, 0)) {
    igraph_error("", __FILE__, __LINE__, IGRAPH_ENOMEM);
  }
                                        /* Call igraph */
  ...
  PROTECT(history=R_igraph_vector_int_to_SEXP(&c_history));
  igraph_vector_int_destroy(&c_history);
...
SEXP R_igraph_subisomorphic_lad(SEXP pattern, SEXP target, SEXP domains,
  ...
  // FIXME: Deprecate time_limit
                                        /* Call igraph */
  IGRAPH_R_CHECK(igraph_subisomorphic_lad(&c_pattern, &c_target, (Rf_isNull(domains) ? 0 : &c_domains), &c_iso, (Rf_isNull(map) ? 0 : &c_map), (Rf_isNull(maps) ? 0 : &c_maps), c_induced));
...
/-------------------------------------------*/
SEXP Rx_igraph_graphlets_candidate_basis(SEXP graph, SEXP weights) {
  /* Declarations */
  igraph_t c_graph;
  igraph_vector_t c_weights;
  igraph_vector_int_list_t c_cliques;
