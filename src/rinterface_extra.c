...
SEXP R_igraph_vectorlist_to_SEXP(const igraph_vector_list_t *list);
...
int R_SEXP_to_igraph(SEXP graph, igraph_t *res) {
  ...
  R_SEXP_to_vector_int(VECTOR_ELT(graph, 2), &res->from);
  R_SEXP_to_vector_int(VECTOR_ELT(graph, 3), &res->to);
  R_SEXP_to_vector_int(VECTOR_ELT(graph, 4), &res->oi);
  R_SEXP_to_vector_int(VECTOR_ELT(graph, 5), &res->ii);
  R_SEXP_to_vector_int(VECTOR_ELT(graph, 6), &res->os);
  R_SEXP_to_vector_int(VECTOR_ELT(graph, 7), &res->is);
...
SEXP R_igraph_lattice(SEXP pdimvector, SEXP pnei, SEXP pdirected,
  ...
  R_SEXP_to_vector_int(pdimvector, &dimvector);
...
SEXP R_igraph_barabasi_game(SEXP pn, SEXP ppower, SEXP pm, SEXP poutseq,
  ...
  if (!Rf_isNull(poutseq)) {
    R_SEXP_to_vector_int(poutseq, &outseq);
    myoutseq=&outseq;
  }
...
SEXP R_igraph_degree_sequence_game(SEXP pout_seq, SEXP pin_seq,
  ...
  R_SEXP_to_vector_int(pout_seq, &outseq);
  if (!Rf_isNull(pin_seq)) { R_SEXP_to_vector_int(pin_seq, &inseq); }
...
SEXP R_igraph_motifs_randesu_estimate(SEXP graph, SEXP psize, SEXP pcutprob,
  ...
  if (GET_LENGTH(psample) != 0) {
    R_SEXP_to_vector_int(psample, &sample);
    sampleptr=&sample;
  }
  R_SEXP_to_igraph(graph, &g);
...
SEXP R_igraph_barabasi_aging_game(SEXP pn, SEXP ppa_exp, SEXP paging_exp,
  ...
  R_SEXP_to_vector_int(pout_seq, &out_seq);
...
SEXP R_igraph_layout_reingold_tilford(SEXP graph, SEXP proot,
  ...
  R_SEXP_to_igraph(graph, &g);
  R_SEXP_to_vector_int(proot, &root);
  R_SEXP_to_vector_int(prootlevel, &rootlevel);
  igraph_matrix_init(&res, 0, 0);
  if (!circ) {
...
SEXP R_igraph_vs_nei(SEXP graph, SEXP px, SEXP pv, SEXP pmode) {
  ...
  while (!IGRAPH_VIT_END(vv)) {
    ...
    for (i=0; i<igraph_vector_int_size(&neis); i++) {
      igraph_integer_t nei=igraph_vector_int_get(&neis, i);
...
SEXP R_igraph_es_adj(SEXP graph, SEXP x, SEXP pv, SEXP pmode) {
  ...
  while (!IGRAPH_VIT_END(vv)) {
    ...
    for (i=0; i<igraph_vector_int_size(&adje); i++) {
      igraph_integer_t edge=igraph_vector_int_get(&adje, i);
...
SEXP R_igraph_cited_type_game(SEXP pnodes, SEXP pedges, SEXP ptypes,
  ...
  R_SEXP_to_vector_int(ptypes, &types);
...
SEXP R_igraph_is_chordal(SEXP graph, SEXP alpha, SEXP alpham1,
  ...
  SEXP result, names;
                                        /* Convert input */
  R_SEXP_to_igraph(graph, &c_graph);
  if (!Rf_isNull(alpha)) { R_SEXP_to_vector_int(alpha, &c_alpha); }
  if (!Rf_isNull(alpham1)) { R_SEXP_to_vector_int(alpham1, &c_alpham1); }
...
SEXP R_igraph_bfs(SEXP graph, SEXP proot, SEXP proots, SEXP pneimode,
  ...
  R_SEXP_to_igraph(graph, &g);
  if (!Rf_isNull(proots)) {
    R_SEXP_to_vector_int(proots, &roots);
  }
  if (!Rf_isNull(prestricted)) {
    R_SEXP_to_vector_int(prestricted, &restricted);
  }
...
igraph_error_t R_igraph_dfshandler(const igraph_t *graph,
  ...
  PROTECT(result = R_igraph_safe_eval_in_env(R_fcall, data->rho, NULL));
  cres = Rf_asInteger(R_igraph_handle_safe_eval_result_in_env(result, data->rho));
...
SEXP R_igraph_subisomorphic_lad(SEXP pattern, SEXP target, SEXP domains,
  ...
  R_igraph_SEXP_to_0orvector_int_list(domains, &c_domains);
