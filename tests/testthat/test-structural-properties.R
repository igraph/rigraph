...
test_that("knnk works", {
  ...
  result3 <- knnk(g3, from_mode = "out", to_mode = "in")
  ...
  # Ignore the weight attribute
  expect_equal(knnk(g4, weights = NA), c(NaN, 4))

  ## Test directed_neighbors parameter
  g5 <- make_graph(c(1, 2, 1, 3), directed = TRUE)
  # With directed_neighbors = TRUE (default): edges are directed
  result5a <- knnk(
    g5,
    from_mode = "out",
    to_mode = "in",
  ...
  # With directed_neighbors = FALSE: edges are treated as reciprocal
  result5b <- knnk(
    g5,
    from_mode = "out",
    to_mode = "in",
  ...
  # Results should differ when directed_neighbors changes
...
test_that("knnk errors", {
  g <- make_ring(10)

  # All arguments except the graph must be passed by name
  expect_snapshot_igraph_error(knnk(g, "out"))
