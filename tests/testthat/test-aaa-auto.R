...
test_that("sparse_adjacency_impl works", {
  ...
  # Test the autogenerated function
  result <- sparse_adjacency_impl(mat, mode = "directed", loops = "once")
  
  # Check that we got a graph back
  expect_true(is.list(result))
  expect_true("graph" %in% names(result))
  expect_true(igraph::is_igraph(result$graph))
  
  # Check the structure
  expect_equal(igraph::vcount(result$graph), 3)
  expect_equal(igraph::ecount(result$graph), 4)
})

test_that("weighted_sparsemat_impl works", {
  skip_if_not_installed("Matrix", minimum_version = "1.6.0")
  ...
  # Test the autogenerated function
  result <- weighted_sparsemat_impl(mat, directed = TRUE, attr = "weight", loops = FALSE)
  ...
  # Check the structure
  expect_equal(igraph::vcount(result$graph), 3)
  expect_equal(igraph::ecount(result$graph), 3)
  
  # Check that edges have weights
  expect_true("weight" %in% igraph::edge_attr_names(result$graph))
})

test_that("get_laplacian_sparse_impl works", {
  skip_if_not_installed("Matrix", minimum_version = "1.6.0")
  ...
  # Check that we got a sparse matrix back
  expect_true(is.list(result))
  expect_true("sparseres" %in% names(result))
  expect_true(inherits(result$sparseres, "igraph.tmp.sparse"))
})

test_that("get_adjacency_sparse_impl works", {
  skip_if_not_installed("Matrix", minimum_version = "1.6.0")
  ...
  # Check that we got a sparse matrix back
  expect_true(is.list(result))
  expect_true("sparsemat" %in% names(result))
  expect_true(inherits(result$sparsemat, "igraph.tmp.sparse"))
})

test_that("get_stochastic_sparse_impl works", {
  skip_if_not_installed("Matrix", minimum_version = "1.6.0")
