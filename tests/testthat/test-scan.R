...
test_that("Neighborhoods work for us", {
  ...
  expect_equal(digest::digest(s1), "df0fd77489f70cc47f682dc31d9f52f5")
  s1 <- local_scan(graphs$g, k = 1, weighted = TRUE, neighborhoods = nei)
  expect_equal(digest::digest(s1), "af720916ae4b49881745d2dcdd614401")

  nei <- neighborhood(g, order = 2)
  s1 <- local_scan(graphs$g, k = 2, neighborhoods = nei)
  expect_equal(digest::digest(s1), "6f47f47abde25d00d615dd56826cca5a")
  s1 <- local_scan(graphs$g, k = 2, weighted = TRUE, neighborhoods = nei)
  expect_equal(digest::digest(s1), "e02e9d58168ee5d53850497f6d4c76b0")
})
...
test_that("Neighborhoods work for them", {
  ...
  nei <- neighborhood(g, order = 2)
  s1 <- local_scan(graphs$g, graphs$gp, k = 2, neighborhoods = nei)
  expect_equal(digest::digest(s1), "a3237a9a55e9d86ab471c81a291eb03b")
  s1 <- local_scan(
    graphs$g,
    graphs$gp,
...
test_that("General scan-stat works, US, scan-1, unweighted, directed", {
  ...
  s1i <- local_scan(graphs$g, k = 1, weighted = FALSE, mode = "in")
...
test_that("General scan-stat works, US, scan-1, weighted, directed", {
  ...
  s1i <- local_scan(graphs$g, k = 1, weighted = TRUE, mode = "in")
...
test_that("Issue 18 is resolved", {
  ...
  expect_equal(local_scan(graphs$g, mode = "all"), c(4, 3, 7, 6, 5))
  expect_equal(local_scan(graphs$g, mode = "out"), c(4, 3, 7, 2, 5))
  expect_equal(local_scan(graphs$g, mode = "in"), c(4, 2, 4, 6, 5))
})
...
test_that("Issue 20 is resolved", {
  ...
  ls <- local_scan(graphs$g2, g1, k = 1, mode = "all")
...
test_that("weights argument works", {
  g <- make_ring(10)
  E(g)$weight <- 1:10
  r1 <- local_scan(graphs$g, k = 1, weighted = TRUE, weights = 1:10)
  r2 <- local_scan(graphs$g, k = 1, weighted = TRUE, weights = E(g)$weight)
  expect_equal(r1, r2)
  r3 <- local_scan(graphs$g, k = 1, weighted = FALSE, weights = 1:10)
