test_that("Power centrality works", {
  ...
  expect_that(
    bp.f,
    equals(list(
      c(-1.72, 1.53, -0.57), c(-0.55, 2.03, -0.18),
  ...
  bp.c.w.sparse <- lapply(seq(-.4, .4, by = 0.1), function(x) {
    round(power_centrality(g.c.w, exponent = x), 2)[c(1, 2, 5)]
  })
  expect_that(bp.c.w.sparse, equals(list(
  ...
  bp.c.w.dense <- lapply(seq(-.4, .4, by = 0.1), function(x) {
    round(power_centrality(g.c.w, exponent = x, sparse = FALSE), 2)[c(1, 2, 5)]
  })
  expect_that(bp.c.w.dense, equals(list(
    c(0.75, 0.47, 1.42), c(0.87, -0.69, 1.13),
    c(-0.72, 0.33, -1.38), c(-0.54, 0.61, -1.29),
    c(-0.25, 1.01, -1.01), c(0.15, 1.33, -0.45),
    c(0.69, 1.01, 0.81), c(0.20, 1.52, -0.58),
