...
test_that("union() combines vertex attributes with sum", {
  gs <- make_named_pair()
  u <- union(gs$g1, gs$g2, vertex_attr_comb = "sum")
...
test_that("union() combines edge attributes with sum", {
  gs <- make_named_pair()
  u <- union(gs$g1, gs$g2, edge_attr_comb = "sum")
...
test_that("union() honours per-attribute list spec with rename fallback", {
  ...
  u <- union(
    gs$g1,
    gs$g2,
    vertex_attr_comb = list(weight = "sum", "rename")
  )
  expect_setequal(
    vertex_attr_names(u),
...
test_that("union() can drop clashing attributes with ignore", {
  gs <- make_named_pair()
  u <- union(gs$g1, gs$g2, edge_attr_comb = "ignore")
...
test_that("union() supports custom function combiner", {
  ...
  u <- union(
    gs$g1,
    gs$g2,
    vertex_attr_comb = list(weight = function(x) mean(x))
  )
...
test_that("union() picks first non-NA when only one input has the attr", {
  gs <- make_named_pair()
  u <- union(gs$g1, gs$g2, vertex_attr_comb = "first", byname = TRUE)
  expect_setequal(vertex_attr_names(u), c("name", "weight"))
  expect_equal(
...
test_that("intersection() takes attr.comb args", {
  gs <- make_named_pair()
  i <- intersection(gs$g1, gs$g2, edge_attr_comb = "sum")
...
test_that("compose() takes attr.comb args", {
  ...
  g <- compose(g1, g2, vertex_attr_comb = "sum")
...
test_that("disjoint_union() combines graph attrs via comb", {
  ...
  g1$label <- "first"
  g2$label <- "second"
  u <- disjoint_union(g1, g2, graph_attr_comb = "concat")
...
test_that("simplify() rejects 'rename' combiner", {
  ...
  E(g)$weight <- 1:5
  expect_error(
    simplify(g, edge_attr_comb = "rename"),
    "rename"
  )
})
