...
`make_graph` can create some notable graphs. For example you can create a Frucht graph, who is the smallest cubical graph whose automorphism group consists only of the identity element. It has 12 vertices and 18 edges:
...
You can also include literals here. In this case, the first term of the formula has to start with a '~' character, just like regular formulae in R. The expressions consist of vertex names and edge operators. An edge operator is a sequence of ‘-’ and ‘+’ characters, the former is for the edges and the latter is used for arrow heads. The edges can be arbitrarily long, ie. you may use as many ‘-’ characters to “draw” them as you like. If all edge operators consist of only ‘-’ characters then the graph will be undirected, whereas a single ‘+’ character implies a directed graph:

```{r echo = TRUE}
g <- make_graph( ~ A-B, C-D, E-F, G-H, I, J, K)
g
```
...
If you try to add edges to vertices with invalid IDs (i.e., you try to add an edge to vertex `5` when the graph has only three vertices), you get an error:

```{r echo = TRUE, eval = FALSE}
g <- add_edges(g, edges = c(5,4))
```
...
```{r echo = TRUE}
g <- g %>% add_edges(edges=c(3,1)) %>% add_vertices(3) %>% add_edges(edges=c(3,4, 4,5, 5,6, 6,4))
g
```

We now have an directed graph with 6 vertices and 7 edges. Vertex and edge IDs are always *contiguous*, so if you delete a vertex all subsequent vertices will be renumbered. When a vertex is renumbered, edges are **not** renumbered, but their source and target vertices will. Use `delete_vertices` and `delete_edges` to perform these operations. For instance, to delete the edge connecting vertices `3-4`, get its ID and then delete it:

```{r echo = TRUE}
get.edge.ids(g, c(3,4))
```
...
Let's elaborate another example. We have the task of elaborating a chordal graph. A graph is chordal (or triangulated) if each of its cycles of four or more nodes has a chord, which
is an edge joining two nodes that are not adjacent in the cycle. Our initial graph is:
...
Each vertex represents a person, so we want to store ages, genders and type of connection. The $ operator is a shortcut to get and and set graph attributes. It is shorter and just as readable as `graph_attr` and `set_graph_attr`.
...
plot(g, layout = layout, main = "Our social network - with names as labels and genders as colors")
```
...
```{r}
plot(g, layout= layout, vertex.color=as.factor(V(g)$gender))
```

```{r}
plot(g, layout= layout, vertex.color=c( "yellow", "red")[1+(V(g)$gender == "f")])
```
