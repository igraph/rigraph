...
```{r echo = TRUE}
chordal_graph <- add_edges(g1, c(2,6, 8,7, 5,7, 2,7, 6,1, 7,1))
plot(chordal_graph)
```
...
If you want to save a graph in R with all the attributes use the R's standard `dput()` method and retrieve it later with dget(). You can also just save the R workspace and restore it later.
-->
```
...
plot(g, layout =layout, vertex.size = 20, margin = 0.5,
            vertex.color=c( "red", "yellow")[1+(V(g)$gender == "m")],
