...
However, this call computes the degree for the whole graph first, so it can be slow for large graphs.

A similar syntax is used for most of the structural properties `igraph` can calculate. For vertex properties, the functions accept a vertex ID or a list of vertex IDs (and if they are omitted, the default is the set of all vertices). For edge properties, the functions accept a single edge ID or a list of edge IDs.
...
Imagine that in a given social network, you want to find out who has the largest degree. You can do that with the tools presented so far and the `which` namespace:
...
`which` selections and vertex/edge attributes cam be combined to obtain fairly complex boolean selections. For instance, the following command gives you the names of the individuals younger than 30 years in our social network:
...
Using `.to` filter based on the target vertices. This is different from `.from` if the graph is directed, while it gives the same answer for undirected graphs.
...
Layout functions return a matrix, usually a two column matrix (3 columns for a 3D layout). Each row in this matrix corresponds to a vertex in the original graph, ordered by ID, and contains the vertex coordinates in the 2D or 3D space.

***
**NOTE:** If you are familiary with the Python interface of `igraph`, you might notice that layouts are handled differently there, with a dedicated `Layout` object. In the R interface, they are simple matrices.
