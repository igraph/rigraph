...
# Tutorial
`igraph` is a fast and open source library for the analysis of graphs or networks. The library consists of a core written in C and bindings in a few high-level languages including R. This vignette aims to give you an overview of the functions available in the R interface of `igraph`. For detailed function by function API documentation, check out [https://igraph.org/r/html/latest/](https://igraph.org/r/html/latest/).

Throughout this tutorial, we will use words `graph` and `network` as synonyms, and also `vertex` or `node` as synonyms.
...
Vertices and edges have numerical vertex IDs in igraph. Vertex IDs are always consecutive and they start with 1 (one). For a graph with n vertices the vertex IDs are always between 1 and n. If some operation changes the number of vertices in the graphs, e.g. a subgraph is created via induced_subgraph, then the vertices are renumbered to satisfy this criterion.
...
Let's continue working with the Karate club graph. To add to add one or more vertices to an existing graph, use `add_vertices`:
...
Let us add some more vertices and edges to our graph. In `igraph` we can use the `magrittr` package who provides a mechanism for chasing commands with the operator %\>%:
...
Other function generate graphs *stochastically*, i.e. they produce a different graph each time. For instance `sample_grg`:
...
Each vertex represents a person, so we want to store ages, genders and type of connection between two people (`is_formal` refers to whether a connection between one person or another is formal or informal, i.e. colleagues or friends). The `\$` operator is a shortcut to get and set graph attributes. It is shorter and just as readable as `graph_attr` and `set_graph_attr`.
...
For instance, to look up the vertex corresponding to Carmina, one can do the following:
...
Another way to approach this example would be the following:
...
Instead of specifying the visual properties as vertex and edge attributes, you can also give them as arguments to `plot`. The following plot shows the formal ties with thick lines while informal ones with thin lines:
...
`igraph` can be used to create publication-quality plots. The preferred format is inferred from the file name extension. `igraph` can save to anything that is supported by Cairo, including SVG, PDF and PNG files. SVG or PDF files can then later be converted to PostScript (`.ps`) or Encapsulated PostScript (`.eps`) format if you prefer that, while PNG files can be converted to TIF (`.tif`):
