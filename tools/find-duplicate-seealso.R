...
parse <- function(rd_file) {
  ...
  examples <- xml2::xml_find_first(html, "//p[preceding-sibling::h3[text()='Examples']]")
  xml2::xml_remove(examples)
  see_also <- xml2::xml_find_all(html, "//p[preceding-sibling::h3[text()='See Also']]")
  if (length(see_also) <= 1) return()
