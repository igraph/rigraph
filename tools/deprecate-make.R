...
parse_package_defs <- function() {
  ...
  parse_script_function_call <- function(script, script_name) {
    ...
    parse_function <- function(fn, script_name) {
      ...
      inline <- brio::read_lines(script_name)[(xml2::xml_attr(body, "line1")):xml2::xml_attr(body, "line2")]
...
gert::git_add("tools*")
gert::git_commit("refactor: add script used for refactoring deprecation in make.R")
