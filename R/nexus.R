...
makeNexusDatasetInfo <- function(entries) {
  ...
  if (length(myattr) != 0) {
    ...
    dsi$attributes <- mapply(attrdat, myattr,
      SIMPLIFY = FALSE,
      FUN = function(dat, desc) {
        list(
          type = dat[1], datatype = dat[2], name = dat[3],
          description = desc
        )
      }
    )
...
print.nexusDatasetInfo <- function(x, ...) {
  ...
  attr <- x[["attributes"]]
  printed <- c(
    "id", "sid", "vertices/edges", "name", "tags", "networks",
    "attributes"
  )
...
print.nexusDatasetInfoList <- function(x, ...) {
  ...
  df <- data.frame(
    no = paste(sep = "", "[", format(seq_along(x)), "] "),
    sid = format(sid),
  ...
  out <- paste(
    sep = "", substr(out, 1, getOption("width") - 1),
    ifelse(long, "+", "")
  )
  cat(out, sep = "\n")
  invisible(x)
