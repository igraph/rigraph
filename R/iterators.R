...
get_es_ref <- get_vs_ref <- function(graph) {
  if (is_igraph(graph) && !warn_version(graph)) {
    .Call(Rx_igraph_copy_env, graph) # no _impl
  } else {
    NULL
  }
}
...
identical_graphs <- function(g1, g2, attrs = TRUE) {
  ...
  .Call(Rx_igraph_identical_graphs, g1, g2, as.logical(attrs)) # is_same_graph_impl() has no attrs argument
}
...
`[.igraph.es` <- function(x, ...) {
  ...
  if (is.null(graph)) {
    ...
    bottom <- rlang::new_environment(
      ...
      c(
        attrs,
        .igraph.from = list(.Call(Rx_igraph_copy_from, graph)[as.numeric(x)]), # no _impl
        .igraph.to = list(.Call(Rx_igraph_copy_to, graph)[as.numeric(x)]), # no _impl
