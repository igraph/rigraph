...
disjoint_union <- function(...) {
  ...
  res <- .Call(Rx_igraph_disjoint_union, graphs) # disjoint_union_many_impl does not merge attributes
...
.igraph.graph.union.or.intersection <- function(
  ...
  if (byname) {
    ...
    on.exit(.Call(Rx_igraph_finalizer))
    if (call == "union") {
      res <- .Call(Rx_igraph_union, newgraphs, edgemaps) # union_many_impl does not return edgemaps
    } else {
      res <- .Call(Rx_igraph_intersection, newgraphs, edgemaps) # intersection_many_impl does not return edgemaps
    }
    maps <- res$edgemaps
    res <- res$graph
    ...
    on.exit(.Call(Rx_igraph_finalizer))
    if (call == "union") {
      res <- .Call(Rx_igraph_union, graphs, edgemaps) # union_many_impl does not return edgemaps
    } else {
      res <- .Call(Rx_igraph_intersection, graphs, edgemaps) # intersection_many_impl does not return edgemaps
    }
    maps <- res$edgemaps
    res <- res$graph
...
compose <- function(g1, g2, byname = "auto") {
  ...
  on.exit(.Call(Rx_igraph_finalizer))
  res <- .Call(Rx_igraph_compose, g1, g2, edgemaps) # compose_impl does not return edgemaps
