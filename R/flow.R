...
min_cut <- function(graph, source = NULL, target = NULL, capacity = NULL, value.only = TRUE) {
  ...
  if (is.null(target) && is.null(source)) {
    ...
    if (value.only) {
      res <- .Call(
        C_R_igraph_st_mincut_value, graph,
        as.igraph.vs(graph, source) - 1,
        as.igraph.vs(graph, target) - 1, capacity
      )
    } else {
      res <- .Call(
        C_R_igraph_st_mincut, graph,
        as.igraph.vs(graph, source) - 1,
        as.igraph.vs(graph, target) - 1, capacity
      )
      # No need to add +1 here; C_R_igraph_st_mincut() is autogenerated and
      # adds +1 already
      if (igraph_opt("return.vs.es")) {
        res$cut <- create_es(graph, res$cut)
        res$partition1 <- create_vs(graph, res$partition1)
...
vertex_connectivity <- function(graph, source = NULL, target = NULL, checks = TRUE) {
  ...
  if (is.null(source) && is.null(target)) {
    ...
    on.exit(.Call(C_R_igraph_finalizer))
    .Call(
      C_R_igraph_st_vertex_connectivity, graph, as.igraph.vs(graph, source) - 1,
...
edge_connectivity <- function(graph, source = NULL, target = NULL, checks = TRUE) {
  ...
  if (is.null(source) && is.null(target)) {
    ...
    on.exit(.Call(C_R_igraph_finalizer))
    .Call(
      C_R_igraph_st_edge_connectivity, graph,
...
vertex_disjoint_paths <- function(graph, source = NULL, target = NULL) {
  ...
  on.exit(.Call(C_R_igraph_finalizer))
  .Call(
    C_R_igraph_vertex_disjoint_paths, graph, as.igraph.vs(graph, source) - 1,
    as.igraph.vs(graph, target) - 1
  )
}
