...
#' `r lifecycle::badge("deprecated")`
#'
#' `vertex.connectivity()` was renamed to [vertex_connectivity()] to create a more consistent API.
#' @inheritParams vertex_connectivity
#' @keywords internal
#' @export
vertex.connectivity <- function(
  graph,
  source = NULL,
  target = NULL,
  checks = TRUE
...
#' `r lifecycle::badge("deprecated")`
#'
#' `minimum.size.separators()` was renamed to [min_separators()] to create a more consistent API.
...
#' `r lifecycle::badge("deprecated")`
#'
#' `graph.mincut()` was renamed to [min_cut()] to create a more consistent API.
#' @inheritParams min_cut
#' @keywords internal
#' @export
graph.mincut <- function(
  graph,
  source = NULL,
  target = NULL,
  capacity = NULL,
...
#' `r lifecycle::badge("deprecated")`
#'
#' `graph.adhesion()` was renamed to [adhesion()] to create a more consistent API.
...
#' `r lifecycle::badge("deprecated")`
#'
#' `edge.connectivity()` was renamed to [edge_connectivity()] to create a more consistent API.
#' @inheritParams edge_connectivity
#' @keywords internal
#' @export
edge.connectivity <- function(
  graph,
  source = NULL,
  target = NULL,
  checks = TRUE
...
#'
#' The minimum cut of a graph is the minimum total weight of the edges needed to remove to separate the graph into (at least) two components.
#' (Which is to make the graph *not* strongly connected in the directed case.)
#'
#' The maximum flow between two vertices in a graph is the same as the minimum st-cut,
#' so `max_flow()` and `min_cut()` essentially calculate the same quantity,
#' the only difference is that `min_cut()` can be invoked without giving the `source` and `target` arguments and then minimum of all possible minimum cuts is calculated.
#'
...
#' @param capacity Vector giving the capacity of the edges.
#'   If this is `NULL` (the default) then the `capacity` edge attribute is used.
#' @param value.only Logical, if `TRUE` only the minimum cut value is returned,
#'   if `FALSE` the edges in the cut and a the two (or more) partitions are also returned.
#' @return For `min_cut()` a nuieric constant, the value of the minimum cut, except if `value.only = FALSE`.
#'   In this case a named list with components:
#'   \describe{
#'     \item{value}{
...
#'       (after the cut edges are removed),
#'       as the graph may fall apart into more than two components.
#'     }
#'     \item{partition2}{
...
#' assuming no direct edges between them.
#'
...
#' The maximum number of vertex disjoint paths between two vertices is the same as their vertex connectivity in most cases (if the two vertices are not connected by an edge).
#'
#' The cohesion of a graph (as defined by White and Harary, see references), is the vertex connectivity of the graph.
#' This is calculated by `cohesion()`.
#'
#' These three functions essentially calculate the same measure(s), more precisely `vertex_connectivity()` is the most general,
#' the other two are included only for the ease of using more descriptive function names.
#'
#' @aliases cohesion
#' @param graph,x The input graph.
#' @param source The ID of the source vertex, for `vertex_connectivity()` it can be `NULL`, see details below.
#' @param target The ID of the target vertex, for `vertex_connectivity()` it can be `NULL`, see details below.
#' @param \dots For `vertex_connectivity()`, these dots must be empty.
#'   For `cohesion()`, unused, present for S3 method consistency but may be used by other methods that implement it.
#' @param checks Logical.
...
#'   It is a good idea to perform these checks, as they can be done quickly compared to the connectivity calculation itself.
...
#' `edge_connectivity()` calculates this quantity if neither the `source` nor the `target` arguments are given (i.e. they are both `NULL`).
#'
...
#' The maximum number of edge-disjoint paths are calculated by this function using maximum flow techniques.
...
#'
#' When there are no direct edges between the source and the target,
#' the number of vertex-disjoint paths is the same as the vertex connectivity of the two vertices.
#' When some edges are present, each one of them contributes one extra path.
#'
...
#' @param graph The input graph.
#' @param source The ID of the source vertex, for `edge_connectivity()` it can be `NULL`, see details below.
#' @param target The ID of the target vertex, for `edge_connectivity()` it can be `NULL`, see details below.
#' @inheritParams rlang::args_dots_empty
#' @param checks Logical.
#'   Whether to check that the graph is connected and also the degree of the vertices.
#'   If the graph is not (strongly) connected then the connectivity is obviously zero.
...
#'
#' Given a \eqn{G} directed graph and two, different and non-ajacent vertices, \eqn{s} and \eqn{t}, an \eqn{(s,t)}-cut is a set of edges,
#' such that after removing these edges from \eqn{G} there is no directed path from \eqn{s} to \eqn{t}.
#'
#' @param graph The input graph.
#'   It must be directed.
...
#'
#' Listing all minimum \eqn{(s,t)}-cuts of a directed graph, for given \eqn{s} and \eqn{t}.
#'
#' Given a \eqn{G} directed graph and two, different and non-ajacent vertices, \eqn{s} and \eqn{t}, an \eqn{(s,t)}-cut is a set of edges,
#' such that after removing these edges from \eqn{G} there is no directed path from \eqn{s} to \eqn{t}.
#'
#' The size of an \eqn{(s,t)}-cut is defined as the sum of the capacities (or weights) in the cut.
#' For unweighted (=equally weighted) graphs, this is simply the number of edges.
#'
...
#' @inheritParams rlang::args_dots_empty
#' @param capacity Numeric vector giving the edge capacities.
#'   If this is `NULL` and the graph has a `weight` edge attribute, then this attribute defines the edge capacities.
#'   For forcing unit edge capacities, even for graphs that have a `weight` edge attribute, supply `NA` here.
#' @return A list with entries:
#'   \describe{
#'     \item{value}{
...
#'
#' A flowgraph is a directed graph with a distinguished start (or root) vertex \eqn{r}, such that for any vertex \eqn{v},
#' there is a path from \eqn{r} to \eqn{v}.
#' A vertex \eqn{v} dominates another vertex \eqn{w} (not equal to \eqn{v}), if every path from \eqn{r} to \eqn{w} contains \eqn{v}.
#' Vertex \eqn{v} is the immediate dominator or \eqn{w}, \eqn{v=\textrm{idom}(w)}{v=idom(w)},
#' if \eqn{v} dominates \eqn{w} and every other dominator of \eqn{w} dominates \eqn{v}.
#' The edges \eqn{{(\textrm{idom}(w), w)| w \ne r}}{{(idom(w),w)| w is not r}} form a directed tree, rooted at \eqn{r},
#' called the dominator tree of the graph.
#' Vertex \eqn{v} dominates vertex \eqn{w} if and only if \eqn{v} is an ancestor of \eqn{w} in the dominator tree.
#'
#' This function implements the Lengauer-Tarjan algorithm to construct the dominator tree of a directed graph.
#' For details see the reference below.
#'
#' @param graph A directed graph.
#'   If it is not a flowgraph, and it contains some vertices not reachable from the root vertex,
#'   then these vertices will be collected and returned as part of the result.
#' @param root The ID of the root (or source) vertex, this will be the root of the tree.
#' @inheritParams rlang::args_dots_empty
#' @param mode Constant, must be \sQuote{`in`} or \sQuote{`out`}.
#'   If it is \sQuote{`in`}, then all directions are considered as opposite to the original one in the input graph.
#' @return A list with components:
#'   \describe{
#'     \item{dom}{
#'       A numeric vector giving the immediate dominators for each vertex.
#'       For vertices that are unreachable from the root, it contains `NaN`.
#'       For the root vertex itself it contains minus one.
#'     }
#'     \item{domtree}{
...
#' Minimum size vertex separators
#'
#' List all vertex sets that are minimal \eqn{(s,t)} separators for some \eqn{s} and \eqn{t}, in an undirected graph.
#'
#' A \eqn{(s,t)} vertex separator is a set of vertices, such that after their removal from the graph,
#' there is no path between \eqn{s} and \eqn{t} in the graph.
#'
#' A \eqn{(s,t)} vertex separator is minimal
#' if none of its proper subsets is an \eqn{(s,t)} vertex separator for the same \eqn{s} and \eqn{t}.
#'
#' @param graph The input graph.
#'   It may be directed, but edge directions are ignored.
#' @return A list of numeric vectors.
#'   Each vector contains a vertex set (defined by vertex IDs), each vector is an (s,t) separator of the input graph,
...
#' @section Note:
#' Note that the code below returns `{1, 3}` despite its subset `{1}` being a separator as well.
#' This is because `{1, 3}` is minimal with respect to separating vertices 2 and 4.
#'
#' ```{r, eval=FALSE}
...
#'
#' `max_flow()` calculates the maximum flow between two vertices in a weighted (i.e. valued) graph.
#' A flow from `source` to `target` is an assignment of non-negative real numbers to the edges of the graph, satisfying two properties:
#' (1) for each edge the flow (i.e. the assigned number) is not more than the capacity of the edge (the `capacity` parameter or edge attribute),
#' (2) for every vertex, except the source and the target the incoming flow is the same as the outgoing flow.
#' The value of the flow is the incoming flow of the `target` vertex.
#' The maximum flow is the flow of maximum value.
#'
#' @param graph The input graph.
#' @param source The ID of the source vertex.
#' @param target The ID of the target vertex (sometimes also called sink).
...
#'       `nopush` is the number of push operations,
#'       `norelabel` the number of relabelings,
#'       `nogap` is the number of times the gap heuristics was used,
#'       `nogapnodes` is the total number of gap nodes omitted because of the gap heuristics and
#'       `nobfs` is the number of times a global breadth-first-search update was performed
...
#'
#' `is_separator()` determines whether the supplied vertex set is a vertex separator:
#' A vertex set \eqn{S} is a separator
#' if there are vertices \eqn{u} and \eqn{v} in the graph such that all paths between \eqn{u} and \eqn{v} pass through some vertices in \eqn{S}.
#'
#' @param graph The input graph.
#'   It may be directed, but edge directions are ignored.
#' @param candidate A numeric vector giving the vertex IDs of the candidate separator.
#' @return A Logical, whether the supplied vertex set is a (minimal) vertex separator or not. lists all vertex separator of minimum size.
#' @family flow
#' @examples
#' ring <- make_ring(4)
#' min_st_separators(ring)
...
#'
#' `is_min_separator()` decides whether the supplied vertex set is a minimal vertex separator.
#' A minimal vertex separator is a vertex separator, such that none of its proper subsets are a vertex separator.
#'
#' @param graph The input graph.
#'   It may be directed, but edge directions are ignored.
#' @param candidate A numeric vector giving the vertex IDs of the candidate separator.
#' @return A Logical, whether the supplied vertex set is a (minimal) vertex separator or not.
#' @examples
...
#'
#' This function implements the Kanevsky algorithm for finding all minimal-size vertex separators in an undirected graph.
#' See the reference below for the details.
#'
...
#'   It may be directed, but edge directions are ignored.
#' @return A list of numeric vectors.
#'   Each numeric vector is a vertex separator.
