...
#' `r lifecycle::badge("experimental")`
#'
#' This function searches for isomorphisms between two graphs using the VF2
#' algorithm and calls a user-provided callback function for each isomorphism
#' found.
#'
#' @param graph1 The first graph.
#' @param graph2 The second graph.
...
#' @param callback A function to call for each isomorphism found. The function
#'   should accept two arguments: `map12` (integer vector mapping vertex IDs from
#'   graph1 to graph2, 1-based indexing) and `map21` (integer vector mapping
#'   vertex IDs from graph2 to graph1, 1-based indexing). The function should
#'   return `TRUE` to continue the search or `FALSE` to stop it.
#' @return `NULL`, invisibly. This function is called for its side effects
#'   (calling the callback function for each isomorphism).
#' @seealso [isomorphic()], [count_isomorphisms()]
#'
#' @export
#' @family graph isomorphism
#' @cdocs igraph_get_isomorphisms_vf2_callback
#'
#' @examples
...
#'   count <<- count + 1
#'   if (count >= 5) return(FALSE)  # stop after 5 isomorphisms
...
#' Find subisomorphisms with a callback function
#'
#' @description
#' `r lifecycle::badge("experimental")`
#'
#' This function searches for subisomorphisms of graph1 in graph2 using the VF2
#' algorithm and calls a user-provided callback function for each subisomorphism
#' found. A subisomorphism is an isomorphism between graph1 and a subgraph of graph2.
#'
#' @inheritParams isomorphisms_vf2_callback
#' @return `NULL`, invisibly. This function is called for its side effects
#'   (calling the callback function for each subisomorphism).
#' @seealso [isomorphic()], [isomorphisms_vf2_callback()]
#'
#' @export
#' @family graph isomorphism
#' @cdocs igraph_get_subisomorphisms_vf2_callback
#'
#' @examples
#' # Find a triangle in a larger graph
#' g1 <- make_ring(3)  # triangle
#' g2 <- sample_gnp(20, 0.3)
#'
#' count <- 0
#' subisomorphisms_vf2_callback(g1, g2, callback = function(map12, map21) {
#'   count <<- count + 1
#'   if (count >= 10) return(FALSE)  # stop after 10 subisomorphisms
#'   TRUE  # continue search
#' })
#' cat("Found at least", count, "triangles\n")
