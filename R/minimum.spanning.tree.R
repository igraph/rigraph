...
#' Minimum spanning tree
#'
#' A *spanning tree* of a connected graph is a connected subgraph with the smallest number of edges that includes all vertices of the graph.
#' A graph will have many spanning trees.
#' Among these, the *minimum spanning tree* will have the smallest sum of edge weights.
#'
...
#' @param weights Numeric vector giving the weights of the edges in the graph.
#'   The order is determined by the edge IDs.
#'   This is ignored if the `unweighted` algorithm is chosen.
...
#' @return A graph object with the minimum spanning forest.
#'   To check whether it is a tree, check that the number of its edges is `vcount(graph)-1`.
