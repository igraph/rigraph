...
graph.motifs.no <- function(graph, size = 3, cut.prob = rep(0, size)) {
  # nocov start
  lifecycle::deprecate_warn("2.0.0", "graph.motifs.no()", "count_motifs()")
...
#' `r lifecycle::badge("deprecated")`
#'
#' `graph.motifs.est()` was renamed to [sample_motifs()] to create a more consistent API.
#' @param cut.prob Numeric vector giving the probabilities that the search graph is cut at a certain level.
#'   Its length should be the same as the size of the motif (the `size` argument).
...
graph.motifs.est <- function(
  ...
  # nocov start
  lifecycle::deprecate_warn("2.0.0", "graph.motifs.est()", "sample_motifs()")
  sample_motifs(
    graph = graph,
    size = size,
    cut.prob = cut.prob,
...
graph.motifs <- function(graph, size = 3, cut.prob = rep(0, size)) {
  # nocov start
  lifecycle::deprecate_warn("2.0.0", "graph.motifs()", "motifs()")
...
dyad.census <- function(graph) {
  # nocov start
  lifecycle::deprecate_warn("2.0.0", "dyad.census()", "dyad_census()")
...
#' @param size The size of the motif, currently sizes 3 and 4 are supported in directed graphs and sizes 3 to 6 in undirected graphs.
...
#'   `vids` (integer vector of vertex IDs in the motif) and `isoclass` (the isomorphism class of the motif).
...
#' @return When `callback` is `NULL`, `motifs()` returns a numeric vector, the number of occurrences of each motif in the graph.
#'   The motifs are ordered by their isomorphism classes.
...
#'
#' @param graph Graph object, the input graph.
#' @param size The size of the motif, currently size 3 and 4 are supported in directed graphs and sizes 3-6 in undirected graphs.
#' @inheritParams rlang::args_dots_empty
#' @param cut.prob Numeric vector giving the probabilities that the search graph is cut at a certain level.
#'   Its length should be the same as the size of the motif (the `size` argument).
#'   If `NULL`, the default, no cuts are made.
#' @param sample.size The number of vertices to use as a starting point for finding motifs.
#'   Only used if the `sample` argument is `NULL`.
#'   The default is `ceiling(vcount(graph) / 10)` .
#' @param sample Vertices to use as a starting point for finding motifs.
#'   Default: `NULL`.
#' @return A numeric scalar, an estimate for the total number of motifs in the graph.
#' @seealso [isomorphism_class()]
#'
#' @export
#' @family graph motifs
...
#' Dyad census of a graph
#'
#' Classify dyads in a directed graphs.
...
#'   A warning is given if it is not directed.
...
#' @seealso [triad_census()] for the same classification, but with triples.
...
#'
#' @param graph The input graph, it should be directed.
#'   An undirected graph results a warning, and undefined results.
#' @return A numeric vector, the subgraph counts, in the order given in the above description.
#' @author Gabor Csardi \email{csardi.gabor@@gmail.com}
