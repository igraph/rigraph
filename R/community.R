...
#'   given.
#' @inheritParams rlang::args_dots_empty
#' @param vertex_attr_comb Specifies how to combine the vertex attributes in
...
contract <- function(
  ...
  if (...length() > 0L) {
    .arg_handle <- migrate_recover_args(
      list(...),
      current = list(vertex_attr_comb = vertex_attr_comb),
      recover_new = c("vertex_attr_comb"),
      recover_old = c("vertex.attr.comb"),
      match_names = c("vertex.attr.comb", "vertex_attr_comb"),
      match_to = c("vertex_attr_comb", "vertex_attr_comb"),
      defaults = list(vertex_attr_comb = igraph_opt("vertex_attr_comb")),
