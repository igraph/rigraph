...
#' a possible additional requirement is that all edges are oriented away from a root (out-tree or arborescence) or all edges are oriented towards a root (in-tree or anti-arborescence).
...
#'   \sQuote{all} ignores edge directions; \sQuote{out} requires edges to be oriented outwards from the root,
#'   \sQuote{in} requires edges to be oriented towards the root.
#' @param details Whether to return only whether the graph is a tree (`FALSE`) or also a possible root (`TRUE`)
#' @return When `details` is `FALSE`, a logical value that indicates whether the graph is a tree.
#'   When `details` is `TRUE`, a named list with two entries:
#'   \describe{
#'     \item{res}{
...
#' In the directed case,
#' a possible additional requirement is that edges in each tree are oriented away from the root (out-trees or arborescences) or all edges are oriented towards the root (in-trees or anti-arborescences).
#' This test can be controlled using the mode parameter.
#'
...
#' @return The Prüfer sequence of the graph, represented as a numeric vector of vertex IDs in the sequence.
#'
#' @seealso [make_from_prufer()] to construct a graph from its Prüfer sequence
#' @keywords graphs
#' @examples
#'
#' g <- make_tree(13, 3)
...
#'
#' @param graph The input graph to sample from.
#'   Edge directions are ignored if the graph is directed.
#' @inheritParams rlang::args_dots_empty
#' @param vid When the graph is disconnected, this argument specifies how to handle the situation.
#'   When the argument is zero (the default), the sampling will be performed component-wise, and the result will be a spanning forest.
#'   When the argument contains a vertex ID, only the component containing the given vertex will be processed,
#'   and the result will be a spanning tree of the component of the graph.
#' @return An edge sequence containing the edges of the spanning tree.
#'   Use [subgraph_from_edges()] to extract the corresponding subgraph.
#'
#' @keywords graph
