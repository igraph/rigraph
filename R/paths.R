...
path.length.hist <- function(graph, directed = TRUE) { # nocov start
  lifecycle::deprecate_soft("1.7.0", "path.length.hist()", "distance_table()")
...
maximum.cardinality.search <- function(graph) { # nocov start
  lifecycle::deprecate_soft("1.7.0", "maximum.cardinality.search()", "max_cardinality()")
...
is.dag <- function(graph) { # nocov start
  lifecycle::deprecate_soft("1.7.0", "is.dag()", "is_dag()")
  is_dag(graph = graph)
} # nocov end
## -----------------------------------------------------------------------
##
