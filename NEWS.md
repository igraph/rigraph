...
- `graphlet_basis()` returns a list of vertices in the `"cliques"` component. Replace `.Call(Rx_...)` with `_impl` functions for autogenerated entrypoints (#2470, #2472).
...
- Adapt vendoring code to match duckdb/duckdb-r (#2611).
...
- Use `Rx_` and similar prefixes consistently (#2478).

- Rename `R_` to `Rx_` for all `.Call()` functions outside `aaa-auto.R` (#2476, #2477).

- Remove dead code.

- Use autogenerated `_impl` functions instead of `.Call()` (#2434, #2471).
