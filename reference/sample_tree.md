...
- directed:
  ...
  Whether to create a directed tree. The edges of the tree are oriented
  away from the root.

- method:

  The algorithm to use to generate the tree. ‘prufer’ samples Prüfer
  sequences uniformly and then converts the sampled sequence to a tree.
  ‘lerw’ performs a loop-erased random walk on the complete graph to
  uniformly sampleits spanning trees. (This is also known as Wilson's
