...
- Align the workflow kit with `cynkratemplate`
  ([\#2892](https://github.com/igraph/rigraph/issues/2892)).

#### ci

- Filter OS-restricted packages out of the revdep plan
  ([\#2889](https://github.com/igraph/rigraph/issues/2889)).
...
- Make the revdep driver log’s timing record durable and visible
  ([\#2891](https://github.com/igraph/rigraph/issues/2891)).

### Chore

#### ci

- Retire the pair engine from the revdepx core
  ([\#2885](https://github.com/igraph/rigraph/issues/2885)).

- Re-run the reverse-dependency check (most, depth 2)
  ([\#2883](https://github.com/igraph/rigraph/issues/2883)).

- Breaking change: rm unused argument in
  [`layout_with_mds()`](https://r.igraph.org/reference/layout_with_mds.md)
  ([\#2730](https://github.com/igraph/rigraph/issues/2730),
  [\#2869](https://github.com/igraph/rigraph/issues/2869)).

- Some Jarl fixes
  ([\#2874](https://github.com/igraph/rigraph/issues/2874)).

### Documentation

- Rm deprecations vignette as it is not useful
  ([\#2848](https://github.com/igraph/rigraph/issues/2848),
  [\#2873](https://github.com/igraph/rigraph/issues/2873)).
...
- Upload a shard’s results three times, not once
  ([\#2854](https://github.com/igraph/rigraph/issues/2854)).

#### ci

- Stop reporting a check that never ran as `ok`
  ([\#2849](https://github.com/igraph/rigraph/issues/2849)).

#### ci

- Install the system requirements of the packages being checked
  ([\#2853](https://github.com/igraph/rigraph/issues/2853)).

### Performance

#### ci

- Bigger install chunks, a shard resource sampler, and lower-priority
  checks ([\#2852](https://github.com/igraph/rigraph/issues/2852)).
...
- Check `windows-11-arm` with `error_on = "error"`
  ([\#2821](https://github.com/igraph/rigraph/issues/2821)).

### Features

#### ci

- Commit the `revdep2` report, and re-check what it lists
  ([\#2827](https://github.com/igraph/rigraph/issues/2827)).
...
- Heal `main` — unleak `warnPartialMatchArgs` on R 4.2 and name args at
  [`norm_coords()`](https://r.igraph.org/reference/norm_coords.md)/[`make_star()`](https://r.igraph.org/reference/make_star.md)
  call sites for the lifecycle-errors leg
  ([\#2799](https://github.com/igraph/rigraph/issues/2799)).

- Initialize `c_options` in the Stimulus type definition
  ([\#2792](https://github.com/igraph/rigraph/issues/2792)).

### Features

#### migrations

- Allow head/recoverable prefix overlaps, guarded at run time
  ([\#2787](https://github.com/igraph/rigraph/issues/2787)).

### Continuous integration

- Sync R-CMD-check workflow; drop matrix-env plumbing from custom action
  ([\#2798](https://github.com/igraph/rigraph/issues/2798)).

### Refactoring

- Move optional arguments of games functions behind the ellipsis
  ([\#2760](https://github.com/igraph/rigraph/issues/2760)).

- Move optional arguments of layout functions behind the ellipsis
  ([\#2762](https://github.com/igraph/rigraph/issues/2762)).

- Move optional arguments of make functions behind the ellipsis
  ([\#2761](https://github.com/igraph/rigraph/issues/2761)).

- Move optional arguments of topology functions behind the ellipsis
  ([\#2768](https://github.com/igraph/rigraph/issues/2768)).

- Move optional arguments of conversion functions behind the ellipsis
  ([\#2766](https://github.com/igraph/rigraph/issues/2766)).

- Move optional arguments of hrg-embedding functions behind the ellipsis
  ([\#2771](https://github.com/igraph/rigraph/issues/2771)).

- Move optional arguments of flow functions behind the ellipsis
  ([\#2767](https://github.com/igraph/rigraph/issues/2767)).

- Move optional arguments of plotting functions behind the ellipsis
  ([\#2777](https://github.com/igraph/rigraph/issues/2777)).

- Move optional arguments of interface functions behind the ellipsis
  ([\#2770](https://github.com/igraph/rigraph/issues/2770)).

- Move optional arguments of similarity-efficiency functions behind the
  ellipsis ([\#2776](https://github.com/igraph/rigraph/issues/2776)).

- Move optional arguments of cliques functions behind the ellipsis
  ([\#2769](https://github.com/igraph/rigraph/issues/2769)).

- Move optional arguments of motifs-graphlets functions behind the
  ellipsis ([\#2772](https://github.com/igraph/rigraph/issues/2772)).

- Move optional arguments of misc functions behind the ellipsis
  ([\#2778](https://github.com/igraph/rigraph/issues/2778)).

- Move optional arguments of bipartite-degseq functions behind the
  ellipsis ([\#2774](https://github.com/igraph/rigraph/issues/2774)).

- Move optional arguments of walks-cycles functions behind the ellipsis
  ([\#2775](https://github.com/igraph/rigraph/issues/2775)).

- Move optional arguments of trees functions behind the ellipsis
  ([\#2758](https://github.com/igraph/rigraph/issues/2758)).

- Move optional arguments of operators functions behind the ellipsis
  ([\#2773](https://github.com/igraph/rigraph/issues/2773)).

### Testing

- Cover migrated centrality, community, and centralization signatures
  ([\#2786](https://github.com/igraph/rigraph/issues/2786)).

- Make unasserted lifecycle deprecations fail the test suite (opt-in)
  ([\#2782](https://github.com/igraph/rigraph/issues/2782)).

- Cover migrated structural-properties signatures
  ([\#2785](https://github.com/igraph/rigraph/issues/2785)).

### Uncategorized

- Ci: Run all smoke-test checks even when one fails
  ([\#97](https://github.com/igraph/rigraph/issues/97)).

- Ci: Link the responsible workflow run in snapshot update PRs
  ([\#96](https://github.com/igraph/rigraph/issues/96)).
...
- Remove generated `src/Makevars` in cleanup scripts
  ([\#2746](https://github.com/igraph/rigraph/issues/2746)).

- Fix test issues with new withr version
  ([\#2701](https://github.com/igraph/rigraph/issues/2701),
...
- Auto-update from GitHub Actions
  ([\#2718](https://github.com/igraph/rigraph/issues/2718)).

- Separate and annotate top-level vs helper functions in
  `rinterface_extra.c`
  ([\#2105](https://github.com/igraph/rigraph/issues/2105),
  [\#2708](https://github.com/igraph/rigraph/issues/2708)).

- Auto-update from GitHub Actions
  ([\#2702](https://github.com/igraph/rigraph/issues/2702)).
...
- [`autocurve.edges()`](https://r.igraph.org/reference/autocurve.edges.md)
  is now deprecated with a warning instead of a message.
- [`igraph.version()`](https://r.igraph.org/reference/igraph.version.md)
  is now deprecated with a warning instead of a message.
- [`permute.vertices()`](https://r.igraph.org/reference/permute.vertices.md)
  is now deprecated with a warning instead of a message.
...
- [`tkplot.setcoords()`](https://r.igraph.org/reference/tkplot.setcoords.md)
  is now deprecated with a warning instead of a message.
- [`tkplot.rotate()`](https://r.igraph.org/reference/tkplot.rotate.md)
  is now deprecated with a warning instead of a message.
- [`tkplot.reshape()`](https://r.igraph.org/reference/tkplot.reshape.md)
  is now deprecated with a warning instead of a message.
- [`tkplot.off()`](https://r.igraph.org/reference/tkplot.off.md) is now
  deprecated with a warning instead of a message.
- [`tkplot.getcoords()`](https://r.igraph.org/reference/tkplot.getcoords.md)
  is now deprecated with a warning instead of a message.
- [`tkplot.fit.to.screen()`](https://r.igraph.org/reference/tkplot.fit.to.screen.md)
  is now deprecated with a warning instead of a message.
- [`tkplot.close()`](https://r.igraph.org/reference/tkplot.close.md) is
  now deprecated with a warning instead of a message.
- [`tkplot.center()`](https://r.igraph.org/reference/tkplot.center.md)
  is now deprecated with a warning instead of a message.
- [`tkplot.canvas()`](https://r.igraph.org/reference/tkplot.canvas.md)
  is now deprecated with a warning instead of a message.
- [`are.connected()`](https://r.igraph.org/reference/are.connected.md)
  is now deprecated with a warning instead of a message.
- [`get.shortest.paths()`](https://r.igraph.org/reference/get.shortest.paths.md)
  is now deprecated with a warning instead of a message.
- [`get.diameter()`](https://r.igraph.org/reference/get.diameter.md) is
  now deprecated with a warning instead of a message.
- [`unfold.tree()`](https://r.igraph.org/reference/unfold.tree.md) is
  now deprecated with a warning instead of a message.
- [`topological.sort()`](https://r.igraph.org/reference/topological.sort.md)
  is now deprecated with a warning instead of a message.
- [`shortest.paths()`](https://r.igraph.org/reference/shortest.paths.md)
  is now deprecated with a warning instead of a message.
- [`neighborhood.size()`](https://r.igraph.org/reference/neighborhood.size.md)
  is now deprecated with a warning instead of a message.
- [`is.mutual()`](https://r.igraph.org/reference/is.mutual.md) is now
  deprecated with a warning instead of a message.
- [`is.multiple()`](https://r.igraph.org/reference/is.multiple.md) is
  now deprecated with a warning instead of a message.
- [`is.matching()`](https://r.igraph.org/reference/is.matching.md) is
  now deprecated with a warning instead of a message.
- [`is.loop()`](https://r.igraph.org/reference/is.loop.md) is now
  deprecated with a warning instead of a message.
- [`is.connected()`](https://r.igraph.org/reference/is.connected.md) is
  now deprecated with a warning instead of a message.
- [`induced.subgraph()`](https://r.igraph.org/reference/induced.subgraph.md)
  is now deprecated with a warning instead of a message.
- [`has.multiple()`](https://r.igraph.org/reference/has.multiple.md) is
  now deprecated with a warning instead of a message.
- [`graph.neighborhood()`](https://r.igraph.org/reference/graph.neighborhood.md)
  is now deprecated with a warning instead of a message.
- [`graph.laplacian()`](https://r.igraph.org/reference/graph.laplacian.md)
  is now deprecated with a warning instead of a message.
- [`graph.knn()`](https://r.igraph.org/reference/graph.knn.md) is now
  deprecated with a warning instead of a message.
- [`graph.dfs()`](https://r.igraph.org/reference/graph.dfs.md) is now
  deprecated with a warning instead of a message.
- [`graph.density()`](https://r.igraph.org/reference/graph.density.md)
  is now deprecated with a warning instead of a message.
- [`graph.coreness()`](https://r.igraph.org/reference/graph.coreness.md)
  is now deprecated with a warning instead of a message.
- [`graph.bfs()`](https://r.igraph.org/reference/graph.bfs.md) is now
  deprecated with a warning instead of a message.
- [`farthest.nodes()`](https://r.igraph.org/reference/farthest.nodes.md)
  is now deprecated with a warning instead of a message.
- [`count.multiple()`](https://r.igraph.org/reference/count.multiple.md)
  is now deprecated with a warning instead of a message.
- [`clusters()`](https://r.igraph.org/reference/clusters.md) is now
  deprecated with a warning instead of a message.
- [`average.path.length()`](https://r.igraph.org/reference/average.path.length.md)
  is now deprecated with a warning instead of a message.
- [`bfs()`](https://r.igraph.org/reference/bfs.md) is now defunct
  (errors instead of warning).
- [`dfs()`](https://r.igraph.org/reference/dfs.md) is now defunct
  (errors instead of warning).
- [`get.stochastic()`](https://r.igraph.org/reference/get.stochastic.md)
  is now deprecated with a warning instead of a message.
- [`is.simple()`](https://r.igraph.org/reference/is.simple.md) is now
  deprecated with a warning instead of a message.
- [`igraph.shape.noplot()`](https://r.igraph.org/reference/igraph.shape.noplot.md)
  is now deprecated with a warning instead of a message.
- [`igraph.shape.noclip()`](https://r.igraph.org/reference/igraph.shape.noclip.md)
  is now deprecated with a warning instead of a message.
- [`vertex.shapes()`](https://r.igraph.org/reference/vertex.shapes.md)
  is now deprecated with a warning instead of a message.
- [`add.vertex.shape()`](https://r.igraph.org/reference/add.vertex.shape.md)
  is now deprecated with a warning instead of a message.
- [`path.length.hist()`](https://r.igraph.org/reference/path.length.hist.md)
  is now deprecated with a warning instead of a message.
- [`is.dag()`](https://r.igraph.org/reference/is.dag.md) is now
  deprecated with a warning instead of a message.
- [`igraph.options()`](https://r.igraph.org/reference/igraph.options.md)
  is now deprecated with a warning instead of a message.
- [`getIgraphOpt()`](https://r.igraph.org/reference/getIgraphOpt.md) is
  now deprecated with a warning instead of a message.
- [`running.mean()`](https://r.igraph.org/reference/running.mean.md) is
  now deprecated with a warning instead of a message.
- [`igraph.sample()`](https://r.igraph.org/reference/igraph.sample.md)
  is now deprecated with a warning instead of a message.
- [`convex.hull()`](https://r.igraph.org/reference/convex.hull.md) is
  now deprecated with a warning instead of a message.
- [`graph.intersection()`](https://r.igraph.org/reference/graph.intersection.md)
  is now deprecated with a warning instead of a message.
- [`graph.union()`](https://r.igraph.org/reference/graph.union.md) is
  now deprecated with a warning instead of a message.
- [`graph.difference()`](https://r.igraph.org/reference/graph.difference.md)
  is now deprecated with a warning instead of a message.
- [`graph.compose()`](https://r.igraph.org/reference/graph.compose.md)
  is now deprecated with a warning instead of a message.
- [`graph.complementer()`](https://r.igraph.org/reference/graph.complementer.md)
  is now deprecated with a warning instead of a message.
- [`triad.census()`](https://r.igraph.org/reference/triad.census.md) is
  now deprecated with a warning instead of a message.
- [`graph.motifs.no()`](https://r.igraph.org/reference/graph.motifs.no.md)
  is now deprecated with a warning instead of a message.
- [`graph.motifs.est()`](https://r.igraph.org/reference/graph.motifs.est.md)
  is now deprecated with a warning instead of a message.
- [`graph.motifs()`](https://r.igraph.org/reference/graph.motifs.md) is
  now deprecated with a warning instead of a message.
- [`dyad.census()`](https://r.igraph.org/reference/dyad.census.md) is
  now deprecated with a warning instead of a message.
- [`minimum.spanning.tree()`](https://r.igraph.org/reference/minimum.spanning.tree.md)
  is now deprecated with a warning instead of a message.
- [`graph()`](https://r.igraph.org/reference/graph.md) is now deprecated
  with a warning instead of a message.
- [`graph.famous()`](https://r.igraph.org/reference/graph.famous.md) is
  now deprecated with a warning instead of a message.
- [`line.graph()`](https://r.igraph.org/reference/line.graph.md) is now
  deprecated with a warning instead of a message.
- [`graph.ring()`](https://r.igraph.org/reference/graph.ring.md) is now
  deprecated with a warning instead of a message.
- [`graph.tree()`](https://r.igraph.org/reference/graph.tree.md) is now
  deprecated with a warning instead of a message.
- [`graph.star()`](https://r.igraph.org/reference/graph.star.md) is now
  deprecated with a warning instead of a message.
- [`graph.lcf()`](https://r.igraph.org/reference/graph.lcf.md) is now
  deprecated with a warning instead of a message.
- [`graph.lattice()`](https://r.igraph.org/reference/graph.lattice.md)
  is now deprecated with a warning instead of a message.
- [`graph.lattice()`](https://r.igraph.org/reference/graph.lattice.md)
  is now deprecated with a warning instead of a message.
- [`graph.kautz()`](https://r.igraph.org/reference/graph.kautz.md) is
  now deprecated with a warning instead of a message.
- [`graph.full.citation()`](https://r.igraph.org/reference/graph.full.citation.md)
  is now deprecated with a warning instead of a message.
- [`graph.full.bipartite()`](https://r.igraph.org/reference/graph.full.bipartite.md)
  is now deprecated with a warning instead of a message.
- [`graph.full()`](https://r.igraph.org/reference/graph.full.md) is now
  deprecated with a warning instead of a message.
- [`graph.formula()`](https://r.igraph.org/reference/graph.formula.md)
  is now deprecated with a warning instead of a message.
- [`graph.extended.chordal.ring()`](https://r.igraph.org/reference/graph.extended.chordal.ring.md)
  is now deprecated with a warning instead of a message.
- [`graph.empty()`](https://r.igraph.org/reference/graph.empty.md) is
  now deprecated with a warning instead of a message.
- [`graph.de.bruijn()`](https://r.igraph.org/reference/graph.de.bruijn.md)
  is now deprecated with a warning instead of a message.
- [`graph.bipartite()`](https://r.igraph.org/reference/graph.bipartite.md)
  is now deprecated with a warning instead of a message.
- [`graph.atlas()`](https://r.igraph.org/reference/graph.atlas.md) is
  now deprecated with a warning instead of a message.
- [`graph_()`](https://r.igraph.org/reference/graph_.md) is now
  deprecated with a warning instead of a message.
- [`make_lattice()`](https://r.igraph.org/reference/make_lattice.md) is
  now deprecated with a warning instead of a message.
- `layout.grid.3d()` has been removed.
- [`piecewise.layout()`](https://r.igraph.org/reference/piecewise.layout.md)
  is now deprecated with a warning instead of a message.
- [`layout.sugiyama()`](https://r.igraph.org/reference/layout.sugiyama.md)
  is now deprecated with a warning instead of a message.
- [`layout.star()`](https://r.igraph.org/reference/layout.star.md) is
  now deprecated with a warning instead of a message.
- [`layout.norm()`](https://r.igraph.org/reference/layout.norm.md) is
  now deprecated with a warning instead of a message.
- [`layout.merge()`](https://r.igraph.org/reference/layout.merge.md) is
  now deprecated with a warning instead of a message.
- [`layout.mds()`](https://r.igraph.org/reference/layout.mds.md) is now
  deprecated with a warning instead of a message.
- [`layout.grid()`](https://r.igraph.org/reference/layout.grid.md) is
  now deprecated with a warning instead of a message.
- [`layout.graphopt()`](https://r.igraph.org/reference/layout.graphopt.md)
  is now deprecated with a warning instead of a message.
- [`layout.gem()`](https://r.igraph.org/reference/layout.gem.md) is now
  deprecated with a warning instead of a message.
- [`layout.davidson.harel()`](https://r.igraph.org/reference/layout.davidson.harel.md)
  is now deprecated with a warning instead of a message.
- [`layout.bipartite()`](https://r.igraph.org/reference/layout.bipartite.md)
  is now deprecated with a warning instead of a message.
- [`layout.auto()`](https://r.igraph.org/reference/layout.auto.md) is
  now deprecated with a warning instead of a message.
- [`layout.reingold.tilford()`](https://r.igraph.org/reference/layout.reingold.tilford.md)
  is now deprecated with a warning instead of a message.
- [`layout.circle()`](https://r.igraph.org/reference/layout.circle.md)
  is now deprecated with a warning instead of a message.
- [`layout.sphere()`](https://r.igraph.org/reference/layout.sphere.md)
  is now deprecated with a warning instead of a message.
- [`layout.random()`](https://r.igraph.org/reference/layout.random.md)
  is now deprecated with a warning instead of a message.
- [`layout.fruchterman.reingold()`](https://r.igraph.org/reference/layout.fruchterman.reingold.md)
  is now deprecated with a warning instead of a message.
- [`layout.kamada.kawai()`](https://r.igraph.org/reference/layout.kamada.kawai.md)
  is now deprecated with a warning instead of a message.
- [`layout.lgl()`](https://r.igraph.org/reference/layout.lgl.md) is now
  deprecated with a warning instead of a message.
- [`layout_with_mds()`](https://r.igraph.org/reference/layout_with_mds.md)
  is now deprecated with a warning instead of a message.
- [`layout.spring()`](https://r.igraph.org/reference/layout.spring.md)
  is now defunct (errors instead of warning).
- [`layout.svd()`](https://r.igraph.org/reference/layout.svd.md) is now
  defunct (errors instead of warning).
- [`layout.fruchterman.reingold.grid()`](https://r.igraph.org/reference/layout.fruchterman.reingold.grid.md)
  is now defunct (errors instead of warning).
- [`layout.drl()`](https://r.igraph.org/reference/layout.drl.md) is now
  deprecated with a warning instead of a message.
...
- `from()` has been removed.
- `to()` has been removed.
- [`is.directed()`](https://r.igraph.org/reference/is.directed.md) is
  now deprecated with a warning instead of a message.
- [`delete.vertices()`](https://r.igraph.org/reference/delete.vertices.md)
  is now deprecated with a warning instead of a message.
- [`delete.edges()`](https://r.igraph.org/reference/delete.edges.md) is
  now deprecated with a warning instead of a message.
- [`add.vertices()`](https://r.igraph.org/reference/add.vertices.md) is
  now deprecated with a warning instead of a message.
- [`add.edges()`](https://r.igraph.org/reference/add.edges.md) is now
  deprecated with a warning instead of a message.
- [`get_edge_ids()`](https://r.igraph.org/reference/get_edge_ids.md) is
  now defunct (errors instead of warning).
- [`get.edge.ids()`](https://r.igraph.org/reference/get.edge.ids.md) is
  now deprecated with a warning instead of a message.
- [`get.edge.ids()`](https://r.igraph.org/reference/get.edge.ids.md) is
  now deprecated with a warning instead of a message.
- [`graph.incidence()`](https://r.igraph.org/reference/graph.incidence.md)
  is now deprecated with a warning instead of a message.
- [`graph_from_incidence_matrix()`](https://r.igraph.org/reference/graph_from_incidence_matrix.md)
  is now deprecated with a warning instead of a message.
- [`graph_from_incidence_matrix()`](https://r.igraph.org/reference/graph_from_incidence_matrix.md)
  is now deprecated with a warning instead of a message.
- [`hrg.predict()`](https://r.igraph.org/reference/hrg.predict.md) is
  now deprecated with a warning instead of a message.
- [`hrg.fit()`](https://r.igraph.org/reference/hrg.fit.md) is now
  deprecated with a warning instead of a message.
- [`hrg.game()`](https://r.igraph.org/reference/hrg.game.md) is now
  deprecated with a warning instead of a message.
- [`hrg.dendrogram()`](https://r.igraph.org/reference/hrg.dendrogram.md)
  is now deprecated with a warning instead of a message.
- [`hrg.create()`](https://r.igraph.org/reference/hrg.create.md) is now
  deprecated with a warning instead of a message.
- [`hrg.consensus()`](https://r.igraph.org/reference/hrg.consensus.md)
  is now deprecated with a warning instead of a message.
- [`graphlets.project()`](https://r.igraph.org/reference/graphlets.project.md)
  is now deprecated with a warning instead of a message.
- [`graphlets.candidate.basis()`](https://r.igraph.org/reference/graphlets.candidate.basis.md)
  is now deprecated with a warning instead of a message.
- [`watts.strogatz.game()`](https://r.igraph.org/reference/watts.strogatz.game.md)
  is now deprecated with a warning instead of a message.
- [`static.power.law.game()`](https://r.igraph.org/reference/static.power.law.game.md)
  is now deprecated with a warning instead of a message.
- [`static.fitness.game()`](https://r.igraph.org/reference/static.fitness.game.md)
  is now deprecated with a warning instead of a message.
- [`sbm.game()`](https://r.igraph.org/reference/sbm.game.md) is now
  deprecated with a warning instead of a message.
- [`preference.game()`](https://r.igraph.org/reference/preference.game.md)
  is now deprecated with a warning instead of a message.
- [`lastcit.game()`](https://r.igraph.org/reference/lastcit.game.md) is
  now deprecated with a warning instead of a message.
- [`k.regular.game()`](https://r.igraph.org/reference/k.regular.game.md)
  is now deprecated with a warning instead of a message.
- [`interconnected.islands.game()`](https://r.igraph.org/reference/interconnected.islands.game.md)
  is now deprecated with a warning instead of a message.
- [`grg.game()`](https://r.igraph.org/reference/grg.game.md) is now
  deprecated with a warning instead of a message.
- [`growing.random.game()`](https://r.igraph.org/reference/growing.random.game.md)
  is now deprecated with a warning instead of a message.
- [`forest.fire.game()`](https://r.igraph.org/reference/forest.fire.game.md)
  is now deprecated with a warning instead of a message.
- [`establishment.game()`](https://r.igraph.org/reference/establishment.game.md)
  is now deprecated with a warning instead of a message.
- [`degree.sequence.game()`](https://r.igraph.org/reference/degree.sequence.game.md)
  is now deprecated with a warning instead of a message.
- [`connect.neighborhood()`](https://r.igraph.org/reference/connect.neighborhood.md)
  is now deprecated with a warning instead of a message.
- [`citing.cited.type.game()`](https://r.igraph.org/reference/citing.cited.type.game.md)
  is now deprecated with a warning instead of a message.
- [`cited.type.game()`](https://r.igraph.org/reference/cited.type.game.md)
  is now deprecated with a warning instead of a message.
- [`callaway.traits.game()`](https://r.igraph.org/reference/callaway.traits.game.md)
  is now deprecated with a warning instead of a message.
...
- [`barabasi.game()`](https://r.igraph.org/reference/barabasi.game.md)
  is now deprecated with a warning instead of a message.
- [`ba.game()`](https://r.igraph.org/reference/ba.game.md) is now
  deprecated with a warning instead of a message.
- [`asymmetric.preference.game()`](https://r.igraph.org/reference/asymmetric.preference.game.md)
  is now deprecated with a warning instead of a message.
- [`aging.barabasi.game()`](https://r.igraph.org/reference/aging.barabasi.game.md)
  is now deprecated with a warning instead of a message.
- [`aging.ba.game()`](https://r.igraph.org/reference/aging.ba.game.md)
  is now deprecated with a warning instead of a message.
- [`aging.prefatt.game()`](https://r.igraph.org/reference/aging.prefatt.game.md)
  is now deprecated with a warning instead of a message.
...
- [`sample_bipartite()`](https://r.igraph.org/reference/sample_bipartite.md)
  is now deprecated with a warning instead of a message.
- [`bipartite()`](https://r.igraph.org/reference/sample_bipartite.md) is
  now deprecated with a warning instead of a message.
- [`bipartite()`](https://r.igraph.org/reference/sample_bipartite.md) is
  now deprecated with a warning instead of a message.
- [`write.graph()`](https://r.igraph.org/reference/write.graph.md) is
  now deprecated with a warning instead of a message.
- [`read.graph()`](https://r.igraph.org/reference/read.graph.md) is now
  deprecated with a warning instead of a message.
- [`graph.graphdb()`](https://r.igraph.org/reference/graph.graphdb.md)
  is now deprecated with a warning instead of a message.
- [`vertex.disjoint.paths()`](https://r.igraph.org/reference/vertex.disjoint.paths.md)
  is now deprecated with a warning instead of a message.
- [`vertex.connectivity()`](https://r.igraph.org/reference/vertex.connectivity.md)
  is now deprecated with a warning instead of a message.
- [`stMincuts()`](https://r.igraph.org/reference/stMincuts.md) is now
  deprecated with a warning instead of a message.
- [`stCuts()`](https://r.igraph.org/reference/stCuts.md) is now
  deprecated with a warning instead of a message.
- [`minimum.size.separators()`](https://r.igraph.org/reference/minimum.size.separators.md)
  is now deprecated with a warning instead of a message.
- [`minimal.st.separators()`](https://r.igraph.org/reference/minimal.st.separators.md)
  is now deprecated with a warning instead of a message.
- [`is.separator()`](https://r.igraph.org/reference/is.separator.md) is
  now deprecated with a warning instead of a message.
- [`is.minimal.separator()`](https://r.igraph.org/reference/is.minimal.separator.md)
  is now deprecated with a warning instead of a message.
- [`graph.mincut()`](https://r.igraph.org/reference/graph.mincut.md) is
  now deprecated with a warning instead of a message.
- [`graph.maxflow()`](https://r.igraph.org/reference/graph.maxflow.md)
  is now deprecated with a warning instead of a message.
- [`graph.adhesion()`](https://r.igraph.org/reference/graph.adhesion.md)
  is now deprecated with a warning instead of a message.
- [`edge.disjoint.paths()`](https://r.igraph.org/reference/edge.disjoint.paths.md)
  is now deprecated with a warning instead of a message.
- [`edge.connectivity()`](https://r.igraph.org/reference/edge.connectivity.md)
  is now deprecated with a warning instead of a message.
- [`dominator.tree()`](https://r.igraph.org/reference/dominator.tree.md)
  is now deprecated with a warning instead of a message.
- [`power.law.fit()`](https://r.igraph.org/reference/power.law.fit.md)
  is now deprecated with a warning instead of a message.
- [`is.graphical.degree.sequence()`](https://r.igraph.org/reference/is.graphical.degree.sequence.md)
  is now deprecated with a warning instead of a message.
- [`is.degree.sequence()`](https://r.igraph.org/reference/is.degree.sequence.md)
  is now deprecated with a warning instead of a message.
- [`is.chordal()`](https://r.igraph.org/reference/is.chordal.md) is now
  deprecated with a warning instead of a message.
- [`igraph.to.graphNEL()`](https://r.igraph.org/reference/igraph.to.graphNEL.md)
  is now deprecated with a warning instead of a message.
- [`igraph.from.graphNEL()`](https://r.igraph.org/reference/igraph.from.graphNEL.md)
  is now deprecated with a warning instead of a message.
- [`graph.adjlist()`](https://r.igraph.org/reference/graph.adjlist.md)
  is now deprecated with a warning instead of a message.
- [`get.incidence()`](https://r.igraph.org/reference/get.incidence.md)
  is now deprecated with a warning instead of a message.
- [`get.edgelist()`](https://r.igraph.org/reference/get.edgelist.md) is
  now deprecated with a warning instead of a message.
- [`get.data.frame()`](https://r.igraph.org/reference/get.data.frame.md)
  is now deprecated with a warning instead of a message.
- [`get.adjacency()`](https://r.igraph.org/reference/get.adjacency.md)
  is now deprecated with a warning instead of a message.
- [`get.adjlist()`](https://r.igraph.org/reference/get.adjlist.md) is
  now deprecated with a warning instead of a message.
- [`get.adjedgelist()`](https://r.igraph.org/reference/get.adjedgelist.md)
  is now deprecated with a warning instead of a message.
- `get.adjacency.dense()` is now deprecated with a warning instead of a
  message.
- [`as_adj()`](https://r.igraph.org/reference/as_adj.md) is now
  deprecated with a warning instead of a message.
- [`as_incidence_matrix()`](https://r.igraph.org/reference/as_incidence_matrix.md)
  is now deprecated with a warning instead of a message.
- [`as.directed()`](https://r.igraph.org/reference/as.directed.md) is
  now deprecated with a warning instead of a message.
- [`as.undirected()`](https://r.igraph.org/reference/as.undirected.md)
  is now deprecated with a warning instead of a message.
- [`graph.edgelist()`](https://r.igraph.org/reference/graph.edgelist.md)
  is now deprecated with a warning instead of a message.
- [`graph.data.frame()`](https://r.igraph.org/reference/graph.data.frame.md)
  is now deprecated with a warning instead of a message.
- [`igraph.console()`](https://r.igraph.org/reference/igraph.console.md)
  is now deprecated with a warning instead of a message.
- [`no.clusters()`](https://r.igraph.org/reference/no.clusters.md) is
  now deprecated with a warning instead of a message.
- [`decompose.graph()`](https://r.igraph.org/reference/decompose.graph.md)
  is now deprecated with a warning instead of a message.
- [`cluster.distribution()`](https://r.igraph.org/reference/cluster.distribution.md)
  is now deprecated with a warning instead of a message.
- [`biconnected.components()`](https://r.igraph.org/reference/biconnected.components.md)
  is now deprecated with a warning instead of a message.
- [`articulation.points()`](https://r.igraph.org/reference/articulation.points.md)
  is now deprecated with a warning instead of a message.
- [`create.communities()`](https://r.igraph.org/reference/create.communities.md)
  is now deprecated with a warning instead of a message.
- [`walktrap.community()`](https://r.igraph.org/reference/walktrap.community.md)
  is now deprecated with a warning instead of a message.
- [`spinglass.community()`](https://r.igraph.org/reference/spinglass.community.md)
  is now deprecated with a warning instead of a message.
- [`showtrace()`](https://r.igraph.org/reference/showtrace.md) is now
  deprecated with a warning instead of a message.
- [`optimal.community()`](https://r.igraph.org/reference/optimal.community.md)
  is now deprecated with a warning instead of a message.
- [`multilevel.community()`](https://r.igraph.org/reference/multilevel.community.md)
  is now deprecated with a warning instead of a message.
- [`mod.matrix()`](https://r.igraph.org/reference/mod.matrix.md) is now
  deprecated with a warning instead of a message.
- [`leading.eigenvector.community()`](https://r.igraph.org/reference/leading.eigenvector.community.md)
  is now deprecated with a warning instead of a message.
- [`label.propagation.community()`](https://r.igraph.org/reference/label.propagation.community.md)
  is now deprecated with a warning instead of a message.
- [`is.hierarchical()`](https://r.igraph.org/reference/is.hierarchical.md)
  is now deprecated with a warning instead of a message.
- [`infomap.community()`](https://r.igraph.org/reference/infomap.community.md)
  is now deprecated with a warning instead of a message.
- [`fastgreedy.community()`](https://r.igraph.org/reference/fastgreedy.community.md)
  is now deprecated with a warning instead of a message.
- [`edge.betweenness.community()`](https://r.igraph.org/reference/edge.betweenness.community.md)
  is now deprecated with a warning instead of a message.
- [`dendPlot()`](https://r.igraph.org/reference/dendPlot.md) is now
  deprecated with a warning instead of a message.
- [`cutat()`](https://r.igraph.org/reference/cutat.md) is now deprecated
  with a warning instead of a message.
- [`contract.vertices()`](https://r.igraph.org/reference/contract.vertices.md)
  is now deprecated with a warning instead of a message.
- [`code.length()`](https://r.igraph.org/reference/code.length.md) is
  now deprecated with a warning instead of a message.
...
- [`cluster_label_prop()`](https://r.igraph.org/reference/cluster_label_prop.md)
  is now deprecated with a warning instead of a message.
- [`exportPajek()`](https://r.igraph.org/reference/exportPajek.md) is
  now deprecated with a warning instead of a message.
- [`plotHierarchy()`](https://r.igraph.org/reference/plotHierarchy.md)
  is now deprecated with a warning instead of a message.
- [`maxcohesion()`](https://r.igraph.org/reference/maxcohesion.md) is
  now deprecated with a warning instead of a message.
- [`graph.cohesion()`](https://r.igraph.org/reference/graph.cohesion.md)
  is now deprecated with a warning instead of a message.
- [`cohesive.blocks()`](https://r.igraph.org/reference/cohesive.blocks.md)
  is now deprecated with a warning instead of a message.
- [`blockGraphs()`](https://r.igraph.org/reference/blockGraphs.md) is
  now deprecated with a warning instead of a message.
- [`maximal.independent.vertex.sets()`](https://r.igraph.org/reference/maximal.independent.vertex.sets.md)
  is now deprecated with a warning instead of a message.
- [`maximal.cliques.count()`](https://r.igraph.org/reference/maximal.cliques.count.md)
  is now deprecated with a warning instead of a message.
- [`maximal.cliques()`](https://r.igraph.org/reference/maximal.cliques.md)
  is now deprecated with a warning instead of a message.
- [`largest.independent.vertex.sets()`](https://r.igraph.org/reference/largest.independent.vertex.sets.md)
  is now deprecated with a warning instead of a message.
- [`largest.cliques()`](https://r.igraph.org/reference/largest.cliques.md)
  is now deprecated with a warning instead of a message.
- [`independent.vertex.sets()`](https://r.igraph.org/reference/independent.vertex.sets.md)
  is now deprecated with a warning instead of a message.
- [`independence.number()`](https://r.igraph.org/reference/independence.number.md)
  is now deprecated with a warning instead of a message.
- [`clique.number()`](https://r.igraph.org/reference/clique.number.md)
  is now deprecated with a warning instead of a message.
- [`maximal_ivs()`](https://r.igraph.org/reference/maximal_ivs.md) is
  now deprecated with a warning instead of a message.
- [`centralize.scores()`](https://r.igraph.org/reference/centralize.scores.md)
  is now deprecated with a warning instead of a message.
- [`centralization.evcent.tmax()`](https://r.igraph.org/reference/centralization.evcent.tmax.md)
  is now deprecated with a warning instead of a message.
- [`centralization.evcent()`](https://r.igraph.org/reference/centralization.evcent.md)
  is now deprecated with a warning instead of a message.
- [`centralization.degree.tmax()`](https://r.igraph.org/reference/centralization.degree.tmax.md)
  is now deprecated with a warning instead of a message.
- [`centralization.degree()`](https://r.igraph.org/reference/centralization.degree.md)
  is now deprecated with a warning instead of a message.
- [`centralization.closeness.tmax()`](https://r.igraph.org/reference/centralization.closeness.tmax.md)
  is now deprecated with a warning instead of a message.
- [`centralization.closeness()`](https://r.igraph.org/reference/centralization.closeness.md)
  is now deprecated with a warning instead of a message.
- [`centralization.betweenness.tmax()`](https://r.igraph.org/reference/centralization.betweenness.tmax.md)
  is now deprecated with a warning instead of a message.
- [`centralization.betweenness()`](https://r.igraph.org/reference/centralization.betweenness.md)
  is now deprecated with a warning instead of a message.
- [`centr_degree_tmax()`](https://r.igraph.org/reference/centr_degree_tmax.md)
  is now defunct (errors instead of warning).
- [`centr_eigen()`](https://r.igraph.org/reference/centr_eigen.md) is
  now deprecated with a warning instead of a message.
- [`centr_eigen_tmax()`](https://r.igraph.org/reference/centr_eigen_tmax.md)
  is now deprecated with a warning instead of a message.
- [`subgraph.centrality()`](https://r.igraph.org/reference/subgraph.centrality.md)
  is now deprecated with a warning instead of a message.
- [`page.rank()`](https://r.igraph.org/reference/page.rank.md) is now
  deprecated with a warning instead of a message.
- [`hub.score()`](https://r.igraph.org/reference/hub.score.md) is now
  defunct (errors instead of warning).
- [`authority.score()`](https://r.igraph.org/reference/authority.score.md)
  is now defunct (errors instead of warning).
- [`graph.strength()`](https://r.igraph.org/reference/graph.strength.md)
  is now deprecated with a warning instead of a message.
- [`graph.eigen()`](https://r.igraph.org/reference/graph.eigen.md) is
  now deprecated with a warning instead of a message.
- [`graph.diversity()`](https://r.igraph.org/reference/graph.diversity.md)
  is now deprecated with a warning instead of a message.
- [`evcent()`](https://r.igraph.org/reference/evcent.md) is now
  deprecated with a warning instead of a message.
- [`edge.betweenness()`](https://r.igraph.org/reference/edge.betweenness.md)
  is now deprecated with a warning instead of a message.
- [`bonpow()`](https://r.igraph.org/reference/bonpow.md) is now
  deprecated with a warning instead of a message.
...
- [`estimate_closeness()`](https://r.igraph.org/reference/estimate_closeness.md)
  is now deprecated with a warning instead of a message.
- [`arpack()`](https://r.igraph.org/reference/arpack.md) is now
  deprecated with a warning instead of a message.
- [`spectrum()`](https://r.igraph.org/reference/spectrum.md) is now
  deprecated with a warning instead of a message.
...
- [`eigen_centrality()`](https://r.igraph.org/reference/eigen_centrality.md)
  is now defunct (errors instead of warning).
- [`authority_score()`](https://r.igraph.org/reference/hub_score.md) is
  now deprecated with a warning instead of a message.
- [`arpack_defaults()`](https://r.igraph.org/reference/arpack.md) is now
  deprecated with a warning instead of a message.
- [`hub_score()`](https://r.igraph.org/reference/hub_score.md) is now
  deprecated with a warning instead of a message.
- [`arpack_defaults()`](https://r.igraph.org/reference/arpack.md) is now
  deprecated with a warning instead of a message.
- [`bipartite.projection.size()`](https://r.igraph.org/reference/bipartite.projection.size.md)
  is now deprecated with a warning instead of a message.
- [`bipartite.projection()`](https://r.igraph.org/reference/bipartite.projection.md)
  is now deprecated with a warning instead of a message.
- [`bipartite.mapping()`](https://r.igraph.org/reference/bipartite.mapping.md)
  is now deprecated with a warning instead of a message.
- `get.edge()` has been removed.
- [`is.igraph()`](https://r.igraph.org/reference/is.igraph.md) is now
  deprecated with a warning instead of a message.
- [`set.vertex.attribute()`](https://r.igraph.org/reference/set.vertex.attribute.md)
  is now deprecated with a warning instead of a message.
- [`set.graph.attribute()`](https://r.igraph.org/reference/set.graph.attribute.md)
  is now deprecated with a warning instead of a message.
- [`set.edge.attribute()`](https://r.igraph.org/reference/set.edge.attribute.md)
  is now deprecated with a warning instead of a message.
- [`remove.vertex.attribute()`](https://r.igraph.org/reference/remove.vertex.attribute.md)
  is now deprecated with a warning instead of a message.
- [`remove.graph.attribute()`](https://r.igraph.org/reference/remove.graph.attribute.md)
  is now deprecated with a warning instead of a message.
- [`remove.edge.attribute()`](https://r.igraph.org/reference/remove.edge.attribute.md)
  is now deprecated with a warning instead of a message.
- [`list.vertex.attributes()`](https://r.igraph.org/reference/list.vertex.attributes.md)
  is now deprecated with a warning instead of a message.
- [`list.graph.attributes()`](https://r.igraph.org/reference/list.graph.attributes.md)
  is now deprecated with a warning instead of a message.
- [`list.edge.attributes()`](https://r.igraph.org/reference/list.edge.attributes.md)
  is now deprecated with a warning instead of a message.
- [`is.weighted()`](https://r.igraph.org/reference/is.weighted.md) is
  now deprecated with a warning instead of a message.
- [`is.named()`](https://r.igraph.org/reference/is.named.md) is now
  deprecated with a warning instead of a message.
- [`is.bipartite()`](https://r.igraph.org/reference/is.bipartite.md) is
  now deprecated with a warning instead of a message.
- [`get.vertex.attribute()`](https://r.igraph.org/reference/get.vertex.attribute.md)
  is now deprecated with a warning instead of a message.
- [`get.graph.attribute()`](https://r.igraph.org/reference/get.graph.attribute.md)
  is now deprecated with a warning instead of a message.
- [`get.edge.attribute()`](https://r.igraph.org/reference/get.edge.attribute.md)
  is now deprecated with a warning instead of a message.
- [`assortativity.nominal()`](https://r.igraph.org/reference/assortativity.nominal.md)
  is now deprecated with a warning instead of a message.
- [`assortativity.degree()`](https://r.igraph.org/reference/assortativity.degree.md)
  is now deprecated with a warning instead of a message.
...
- [`assortativity()`](https://r.igraph.org/reference/assortativity.md)
  is now deprecated with a warning instead of a message.
- [`ape::as.phylo()`](https://rdrr.io/pkg/ape/man/as.phylo.html) is now
  deprecated with a warning instead of a message.
- [`graph.adjacency()`](https://r.igraph.org/reference/graph.adjacency.md)
  is now deprecated with a warning instead of a message.
...
- Add
  ...
  ([\#1350](https://github.com/igraph/rigraph/issues/1350),
...
- Add
  ...
  ([\#1379](https://github.com/igraph/rigraph/issues/1379),
...
- Add [`mean_degree()`](https://r.igraph.org/reference/degree.md)
...
- Add
  ...
  function ([\#1349](https://github.com/igraph/rigraph/issues/1349),
...
- [`graphlet_basis()`](https://r.igraph.org/reference/graphlet_basis.md)
  returns a list of vertices in the `"cliques"` component instead of a
  raw integer matrix
  ([\#2470](https://github.com/igraph/rigraph/issues/2470),
  [\#2472](https://github.com/igraph/rigraph/issues/2472)).

- [`vertices()`](https://r.igraph.org/reference/vertex.md) now errors on
...
- Implement generic mechanism for
  ...
  allowing layout algorithms to be composed and adjusted uniformly
...
- Error call in
  ...
  [`make_()`](https://r.igraph.org/reference/make_.md) function, not the
  internal helper
  ([\#2609](https://github.com/igraph/rigraph/issues/2609)).

- Not all loops are plotted when a graph has multiple loops at the same
  vertex ([\#2594](https://github.com/igraph/rigraph/issues/2594),
  [\#2595](https://github.com/igraph/rigraph/issues/2595)).

- Fix `hrg_game_impl()` which produced incorrect results
...
- [`graphlet_proj()`](https://r.igraph.org/reference/graphlet_basis.md)
  no longer erroneously assigns class `"igraphHRG"` to the returned
  graph.

- Support character labels in
  [`assortativity_nominal()`](https://r.igraph.org/reference/assortativity.md)
  in addition to integer type codes
...
- Fix uninitialized variable warnings when building with R 4.5 on
  Windows ([\#2526](https://github.com/igraph/rigraph/issues/2526),
  [\#2527](https://github.com/igraph/rigraph/issues/2527)).

- [`modularity()`](https://r.igraph.org/reference/modularity.igraph.md)
  now uses the `"weights"` edge attribute automatically when no
  `weights` argument is supplied.

- Fix
  [`alpha_centrality()`](https://r.igraph.org/reference/alpha_centrality.md)
  crash when `weights` is set to a custom edge attribute name rather
...
- Error messages throughout the package now use the `cli` package for
  consistent, styled output instead of base R
  [`stop()`](https://rdrr.io/r/base/stop.html)
  ([\#731](https://github.com/igraph/rigraph/issues/731),
  [\#2588](https://github.com/igraph/rigraph/issues/2588)).

- The R Consortium is listed as a funder with its ROR ID in the package
  metadata ([\#2587](https://github.com/igraph/rigraph/issues/2587)).

- Fixes to the pkgdown reference index
...
- Documentation pages for R functions now automatically link to the
  corresponding C library documentation
  ([\#2518](https://github.com/igraph/rigraph/issues/2518)).

### Testing

- Structured unit tests alongside snapshot tests have been added for all
  autogenerated `_impl` functions in `test-aaa-auto.R`, verifying
...
- The R interface for most C library functions is now autogenerated
  using the py-stimulus framework, covering 108+ functions including BFS
  and DFS callbacks, leading-eigenvector community detection, motif
  callbacks, sparse matrix operations, and graph-list constructors.
  Arguments use snake_case naming throughout for consistency
  ...
  [\#2484](https://github.com/igraph/rigraph/issues/2484),
  [\#2485](https://github.com/igraph/rigraph/issues/2485),
  [\#2531](https://github.com/igraph/rigraph/issues/2531),
  [\#2532](https://github.com/igraph/rigraph/issues/2532),
  ...
  [\#2552](https://github.com/igraph/rigraph/issues/2552),
...
- Most direct [`.Call()`](https://rdrr.io/r/base/CallExternal.html)
  invocations to C library functions have been replaced with
  autogenerated `_impl()` wrapper functions that use explicit named
  arguments, covering all R source files (`R/assortativity.R`,
  `R/bipartite.R`, `R/centrality.R`, `R/centralization.R`,
  `R/cliques.R`, `R/coloring.R`, `R/embedding.R`, `R/flow.R`,
  `R/games.R`, `R/layout.R`, `R/make.R`, `R/other.R`, `R/simple.R`,
  `R/structural-properties.R`, `R/topology.R`, `R/trees.R`,
  `R/triangles.R`, and others)
  ...
  [\#2357](https://github.com/igraph/rigraph/issues/2357),
...
- Breaking change: Hard-deprecate `get.edge()` and `layout.grid.3d()`
...
- `get.edge()` is deprecated, use
  [`ends()`](https://r.igraph.org/reference/ends.md) instead. (This was
  already the case for igraph 1.0.0, but we forgot to add a NEWS point
  for it.)
